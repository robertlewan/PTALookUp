...
export default function PickPassiveScreen() {
  const navigation = useNavigation<any>();
  const route = useRoute<any>();
  const onPick: (passiveName: string) => void | Promise<void> = route.params.onPick;
  const [query, setQuery] = useState('');

  const filtered = useMemo(() => {
    if (!query.trim()) return allPassives;
    const q = query.toLowerCase();
    return allPassives.filter((p) => p.name.toLowerCase().includes(q));
  }, [query]);
